...
size_t OrderedCount(const Graph &g) {
  ...
  // populating data structures to identify hub vertices
  NodeID* idxMap {nullptr};
  NodeID* inv_idxMap {nullptr};
  Bitmap hubMap(g.num_nodes());
  hubMap.reset();
  NodeID hubCount {0};
  {
    ...
    auto degThresh = degrees[numHubs];
    /* Forming a map between original IDs and index into local buffer*/
    SlidingQueue<NodeID> queue(numHubs); 
    #pragma omp parallel
    {
    ...
    hubCount   = queue.size();
    idxMap     = new NodeID [g.num_nodes()]();
    inv_idxMap = new NodeID [hubCount]();

    #pragma omp parallel for 
    for (auto q_iter = queue.begin(); q_iter < queue.end(); ++q_iter)
    {
        NodeID vtxID = *q_iter;
        NodeID posID = q_iter - queue.begin();
        idxMap[vtxID] = posID;
        inv_idxMap[posID] = vtxID;
    }
  }
  tm.Stop();
  std::cout << "HubDetect Time (in s) = " << tm.Seconds() << std::endl;
    ...
    for (NodeID v : g.out_neigh(u)) {
      ...
      for (NodeID w : g.out_neigh(v)) {
        ...
        if (w == *it)
        {
          total++;
          if (hubMap.get_bit(u))
            ++triangles_dup[omp_get_thread_num()][idxMap[u]];
          else
            fetch_and_add(triangles[u], 1);

          if (hubMap.get_bit(v))
            ++triangles_dup[omp_get_thread_num()][idxMap[v]];
          else
            fetch_and_add(triangles[v], 1);
        
          if (hubMap.get_bit(w))
            ++triangles_dup[omp_get_thread_num()][idxMap[w]];
          else
  ...
  for (NodeID u = 0; u < hubCount; ++u)
  {
    for (int tid = 0; tid < omp_get_max_threads(); ++tid)
    {
      triangles[inv_idxMap[u]] += triangles_dup[tid][u];
    }
  }
